...
public class Reads {
    ...
    private void setAttributes(String line) {
        ...
        if ((attributes[2].contains(".")) && (!attributes[2].startsWith("gi"))) {
            setTarget(attributes[2].split("\\.")[0] + "." + attributes[2].split("\\.")[1] + "." + attributes[2].split("\\.")[2]);
            setLocus(attributes[2].split("\\.")[0]);
        } else if (attributes[2].startsWith("gi")) {
            setTarget(processID(attributes[2]));
            setLocus(processID(attributes[2]));
        } else {
        ...
        if ((attributes[2].contains(".")) && (!attributes[2].startsWith("gi"))) {
            setRefJunction(Integer.parseInt(attributes[2].split("\\.")[3]));
